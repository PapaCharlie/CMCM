...
\label{sec:assumptions}
  ...
  \end{figure}
  As a result of this, we assume that each county can be modeled as a single node in an undirected graph. Here an edge from node $i$ to node $j$ represents that a refugee can directly reach county $j$ from county $i$. Therefore, we constructed our network of Mississippi counties by connecting a county's node to the nodes of all bordering counties. This produced the following graph:
  \begin{figure}[H]
  \centering
  \includegraphics[width=.5\textwidth]{figures/full_undirected.pdf}
