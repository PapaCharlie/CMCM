...
\label{sec:markov}
  ...
  \subsection{Model Description}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=.5\textwidth]{full_directed_ns.pdf}
      \caption{Map of Major Highways in the State of Mississippi \cite{ms_highways}}
    \end{figure}
    Then each edge transition probability in this graph determines the proportion of a county's population that travels to a destination node during a single update. We standardize the time interval of an update to be one hour, and track the number of people in every county after each update in the 4 day period before landfall.
